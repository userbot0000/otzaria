...
class AutoLinkProcessor {
  ...
  String _processTalmudReferences(String htmlText, bool excludeExistingLinks) {
    try {
      ...
      // דפוס לזיהוי הפניות תלמודיות - פשוט ועובד
      // מזהה: [תחילית אופציונלית] + שם מסכת + מספר דף + [עמוד אופציונלי]
      // תומך בגרשיים (כ"ג), סוגריים, פסיקים
      
      final pattern = RegExp(
        '(?:[בדו]ב?\\s*)?' +  // תחילית אופציונלית: ב, ד, וב (לא capturing)
        '(' + tractatePattern + ')' +  // שם המסכת (קבוצה 1)
        '(?![א-ת])' +  // ודא שזו מילה שלמה (לא חלק ממילה גדולה יותר)
        '\\s+' +  // רווחים (חובה לפחות אחד)
        '(?:(דף)\\s+)?' +  // המילה "דף" אופציונלית עם רווח (קבוצה 2)
        '([א-ת]{1,3}(?:[\"\\'][א-ת])?)' +  // מספר הדף עם גרשיים אופציונליים (קבוצה 3)
        '\\s*[,.:;]?\\s*' +  // רווחים ופסיקים אופציונליים (לא capturing)
        '([א-ב]|ע[\"\\'][אב])?',  // עמוד אופציונלי: א, ב, ע"א, ע"ב (קבוצה 4)
        unicode: true,
        multiLine: true,
      );
